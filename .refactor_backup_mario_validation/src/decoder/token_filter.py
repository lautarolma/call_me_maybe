...
- La generación sale de TODOS los ids del vocabulario (~151K en Qwen). Por
    cada step, el filtro reduce ese espacio a tokens que mantienen el output
    como JSON válido (sintaxis + semántica). El camino determinista devuelve un
    set; el camino ambiguo ordena candidatos por logits y los entrega lazily.
...
def compute_allowed_ids(
    ...
    """
    candidate_ids = _candidate_ids(state, vocab)

    # Fases 2 y 3: validación sobre todos los candidatos del camino
    # determinista. Los pasos ambiguos usan iter_ranked_allowed_ids().
...
def iter_ranked_allowed_ids(
    ...
    """Yield valid candidates by logit, then exhaust an exceptional fallback.

    The Top-K is selected from the Phase 1 candidates, not from the full
    vocabulary. Candidates outside that window are considered only after the
    window is exhausted; fallback order is intentionally unspecified.
    """
    candidate_ids = _candidate_ids(state, vocab)
    ...
    for token_id in ranked_ids:
        simulated_ids.add(token_id)
        result = _simulate_allowed_token(state, schema, vocab, trie, token_id)
        if result is not None:
            yield result

    # Correctness fallback: continue searching without claiming logit order.
    for token_id in candidate_ids:
        if token_id in simulated_ids:
            continue
        result = _simulate_allowed_token(state, schema, vocab, trie, token_id)
        if result is not None:
            yield result
...
def _simulate_allowed_token(
    ...
    if decoded is None or not _is_clean_utf8(decoded):
        return None
    valid, new_state = state.simulate(decoded)
    if not valid or not schema.allows_token(decoded, new_state, trie):
        return None
    return token_id, decoded


def _candidate_ids(state: DecoderState, vocab: Vocab) -> set[int]:
    """Return Phase 1 candidates for the current decoder state."""
