...
from src.decoder.token_filter import iter_ranked_allowed_ids
...
class TestFineValidationGap3CompleteKeyValue:
    ...
    def test_fine_validation_can_try_next_ranked_candidate(self) -> None:
        state, schema, vocab, trie = set_params_ctx()
        step(state, schema, '"a": 2.0')
        logits = [-100.0] * vocab.vocab_size
        wrong_type_id = IDS[', "b": "x",']
        right_type_id = IDS[', "b": 4,']
        logits[wrong_type_id] = 10.0
        logits[right_type_id] = 9.0

        candidates = iter_ranked_allowed_ids(
            state, schema, vocab, trie, logits, top_k=2
        )
        first_id, first_text = next(candidates)
        second_id, second_text = next(candidates)

        assert first_id == wrong_type_id
        assert not _passes_fine_validation(
            FUNCTIONS, schema, state, trie, first_text
        )
        assert second_id == right_type_id
        assert _passes_fine_validation(
            FUNCTIONS, schema, state, trie, second_text
        )
